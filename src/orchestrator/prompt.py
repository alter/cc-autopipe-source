...
def _implement_task_prompt_block() -> str:
    return (
        ...
        "\n"
        "## PROMOTION.md leaderboard block (required for promotion-eligible tasks)\n"
        "If your task produces a PROMOTION.md report (any task that closes "
        "with a verdict), include a `## Metrics for leaderboard` section "
        "near the top with labelled fields. The engine reads this block "
        "to populate LEADERBOARD.md; missing fields render the entry "
        "unsortable. Free-form prose / tables elsewhere in the file are "
        "best-effort fallback.\n"
        "\n"
        "Template (omit lines that don't apply to your task type):\n"
        "```\n"
        "## Metrics for leaderboard\n"
        "- **verdict**: PROMOTED | REJECTED | CONDITIONAL | NEUTRAL\n"
        "- **sum_fixed**: <percent as plain number, no % sign>\n"
        "- **regime_parity**: <0..1>\n"
        "- **max_dd**: <negative percent as plain number>\n"
        "- **dm_p_value**: <p-value 0..1>\n"
        "- **dsr**: <Deflated Sharpe Ratio>\n"
        "- **auc**: <ROC AUC 0..1, classification tasks>\n"
        "- **sharpe**: <annualised daily Sharpe, NOT per-bar>\n"
        "```\n"
        "Notes:\n"
        "- `verdict` is mandatory; everything else is per-task.\n"
        "- `sharpe` MUST be the annualised daily value, NOT a per-bar or "
        "inflated estimate.\n"
        "- `**Status**` field (BIAS audit) stays separate — do NOT use "
        "it for the verdict.\n"
    )
